...
class Void_Util_Text {
	 ...
	 * @return string
	 */
	static public function varDump($variable, $html = false) {
		ob_start();
		echo ($html ? '<pre>' : '');
		var_dump($variable);
		echo ($html ? '</pre>' : '');
	 ...
	 * @return string
	 */
	static public function varExport($variable, $html = false) {
		ob_start();
		echo ($html ? '<pre>' : '');
		var_export($variable);
		echo ($html ? '</pre>' : '');
	 ...
	 */
	static public function truncate($string, $max_length, $wordsafe = false, $ellipsis = false, $min_wordsafe_length = 1) {
